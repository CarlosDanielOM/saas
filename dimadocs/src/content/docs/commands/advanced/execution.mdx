...
The execution context supplies the channel, user, plan, command scope, and (for event commands) event payload. EventSub values are only populated for the corresponding event. Function permission checks also depend on the caller: chat and AI actions are checked against the requesting user’s role, while streamer-authored command templates run behind their configured command or event access rules. A command reference does not grant extra permissions.
...
- at most five nested command-reference levels are evaluated
- repeated command references in the same reference path are stopped
