...
import { runChatModeration } from '/app/dist/handlers/moderation.handler.js';
...
for (const [index, expectation] of [['warn', c => c.warn === 'user1'], ['delete', c => c.delete === `msg-${2}`], ['timeout', c => c.ban === 'user1' && c.duration === 60], ['timeout-repeat', c => c.ban === 'user1' && c.duration === 60]].entries()) {
    const result = await runChatModeration(channel, message(`msg-${index + 1}`, 'THIS IS WAY TOO LOUD BRO'), 1);
...
const permitted = await runChatModeration(channel, message('msg-permit', 'STILL ALL CAPS HERE FRIEND'), 1);
...
assert.equal((await runChatModeration(channel, message('msg-mod', 'MORE CAPS THAN ALLOWED HERE', [], 'mod1', 'modone'), 7)).actionTaken, false, 'mods exempt at default level');
assert.equal((await runChatModeration(channel, message('msg-broadcaster', 'I CAN SHOUT ALL I WANT', [], channel, 'fixture'), 10)).actionTaken, false, 'broadcaster never moderated');
assert.equal((await runChatModeration('no-settings-channel', message('msg-none', 'NO SETTINGS AT ALL HERE'), 1)).actionTaken, false, 'missing settings = no moderation');
await ChannelModerationSettingsSchema.create({ channelID: 'disabled-channel', enabled: false, rules });
assert.equal((await runChatModeration('disabled-channel', message('msg-off', 'DISABLED SHOULD NOT FIRE'), 1)).actionTaken, false, 'disabled master switch = no moderation');

// --- Links rule with allowlist ---
assert.equal((await runChatModeration(channel, message('msg-link', 'check scam.xyz/free please', [], 'user2', 'viewer2'), 1)).actionTaken, true, 'non-allowlisted link blocked');
assert.equal((await runChatModeration(channel, message('msg-oklink', 'nice clips.twitch.tv/abc clip', [], 'user3', 'viewer3'), 1)).actionTaken, false, 'allowlisted subdomain passes');

// --- Emote spam rule (fragments, not regex) ---
const emoteFragments = Array.from({ length: 11 }, () => ({ type: 'emote', text: 'KAPPA' }));
assert.equal((await runChatModeration(channel, message('msg-emotes', 'KAPPA '.repeat(11).trim(), emoteFragments, 'user4', 'viewer4'), 1)).actionTaken, true, 'emote spam blocked');
...
assert.equal((await runChatModeration(capsOnly, message('msg-kappa', kappaMessage, Array.from({ length: 4 }, () => ({ type: 'emote', text: 'KAPPA' })), 'user5', 'viewer5'), 1)).actionTaken, false, 'emote text never counts as caps');
...
assert.equal((await runChatModeration(blacklistChannel, message('msg-bl1', 'come get your CAFE SCAM here', [], 'user6', 'viewer6'), 1)).actionTaken, true, 'accent-folded phrase matches');
assert.equal((await runChatModeration(blacklistChannel, message('msg-bl2', 'this is a BADWORD situation', [], 'user6', 'viewer6'), 1)).actionTaken, true, 'case-insensitive term matches');
assert.equal((await runChatModeration(blacklistChannel, message('msg-bl3', 'badwordss should not match', [], 'user7', 'viewer7'), 1)).actionTaken, false, 'whole-word: embedded term does not match');
...
assert.equal((await runChatModeration(channel, message('msg-wide', 'CHANNEL WIDE PERMIT CAPS', [], 'user8', 'viewer8'), 1)).actionTaken, false, 'channel-wide permit bypasses');

console.log(`PASS ${process.env.SAAS_TARGET}: ladder escalation, notices throttled, permits, exemptions, links/emote/blacklist rules, parser contract`);
