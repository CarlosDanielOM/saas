...
const NO_SETTINGS_SENTINEL = '{"none":true}';
...
async function loadSettings(channelID: string): Promise<IChannelModerationSettings | null> {
    ...
    if (!doc) {
        await cache.set(moderationSettingsCacheKey(channelID), NO_SETTINGS_SENTINEL, { EX: SETTINGS_CACHE_TTL_SECONDS });
        return null;
    }
...
/**
 * The moderation gate. Runs inline in the message pipeline after userLevel is
 * computed and before AI/command processing. Detection is synchronous and
 ...
 */
export async function runChatModeration(channelID: string, messageEventData: IChatMessage, userLevel: number): Promise<ChatModerationResult> {
    try {
        ...
        for (const rule of settings.rules) {
            ...
            if (userLevel >= rule.exemptUserLevel) continue;
